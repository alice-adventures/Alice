...
package GitHub.Profile is
   ...
   --  Read the user profile from the user profile file. If the file does not
   --  exist, an exception GitHub_Profile_Error is raised. Mandatory values
   --  are 'login' and 'token': if not present, a GitHub_Profile_Error
   --  exception is raised. Other missing values are set to null strings.
